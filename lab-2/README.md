...
Cree un módulo **entrenos_lectura.py** e implemente en él una función **lee_entrenos**, que reciba la ruta de un fichero en formato CSV codificado en UTF-8 y devuelva una lista de tuplas de tipo Entreno. Utilice la función datetime.strptime con el formato ```%d/%m/%Y %H:%M``` para convertir la fecha. 
...
Cree un módulo **entrenos_funciones.py** e implemente en el funciones que respondan a los siguientes enunciados
a) Dado un año cuál fue el entrenamiento de mayor duración de ese año
b) Dado un tipo devuelva un conjunto con las ciudades donde ha habido ese tipo de entrenamientos.
c) Dado un conjunto de ciudades devuelva el número total de kilómetros recorridos en esas ciudades
d) Dadas dos horas como enteros (sin minutos, por ejemplo, las 10 y las 11) devuelva cuál es la hora de esas dos con mayor número de entrenos.